...
macro if_key key
    push [word @tempIP] bx key
    call @pi_key
    pop bx [word @tempIP]
  endm

macro if_keyDown key
    push [word @tempIP] bx cx si key
    call @pi_keyDown
    pop si cx bx [word @tempIP]
  endm
...
macro ifels_key key, address
    if_key key
    els address
  endm

macro ifels_keyDown key, address
    if_keyDown key
    els address
  endm

macro ifels_keyUp key, address
    if_keyUp key
    els address
  endm

macro ifthen_key key, address
    if_key key
    then address
  endm

macro ifthen_keyDown key, address
    if_keyDown key
    then address
  endm

macro ifthen_keyUp key, address
    if_keyUp key
    then address
  endm
